                            ...
                            <div class="panel-body">
                                ...
                                <div class="tab-content">
                                    <div class="tab-pane fade in active" id="baseinfo">
                                        <form method="post" class="form-horizontal"  role="form" id="currentForm" action="{{ url('simple/ajax/goods_edit_check') }}">
                                                ...
                                                <div class="col-sm-8">
                                                    <select name="category_id" class="form-control m-b">
                                                        ...
                                                        @foreach($category as $key=>$val)
                                                            <option value ="{{$val->id}}" @if($goods->category != null && $val->id == $goods->category->id)selected @endif>{{$val->name}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-8">
                                                    <input type="number" class="form-control" name="stock" disabled value="{{$goods->stock}}">
                                                </div>
                                            </div>
                                    ...
                                    <div class="tab-pane fade in" id="picture">
                                        ...
                                        @include('Simple/Goods/goods_thumb_list')
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
        ...
        $.post(url, data, function (json) {
            if (json.status == -1) {
                ...
                swal({
                    ...
                    window.location.href = "{{asset("simple/goods/goods_list")}}";
                });
            }else{
                swal({
                    title: "提示信息",
    ...
    function uploadForm() {
        ...
        $.ajax({
            url: '{{ url('simple/ajax/upload_thumb_check') }}' ,
            type: 'POST',
            data: formData,
            async: false,
            cache: false,
            ...
            success: function (json) {
                if (json.status == -1) {
                    ...
                    swal({
                        ...
                        //图片添加成功后异步刷新图片列表部分
                        var url = '{{url('simple/ajax/goods_thumb')}}';//需要异步加载的页面
