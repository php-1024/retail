<?php
/**
 * simple_purchase_order表的模型
 *
 */
namespace App\Models;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\SoftDeletes;
class SimplePurchaseOrder extends Model{
    use SoftDeletes;
    protected $table = 'simple_purchase_order';
    protected $primaryKey = 'id';
    public $timestamps = true;
    public $dateFormat = 'U';//设置保存的created_at updated_at为时间戳格式

    //和SimpleSupplier表多对一的关系
    public function SimpleSupplier(){
        return $this->belongsTo('App\Models\SimpleSupplier','company_id','id');
    }
    ...
    public function SimplePurchaseOrderGoods(){
        return $this->hasMany('App\Models\SimplePurchaseOrderGoods','order_id','id');
    }

    //创建订单
    public static function addOrder($param){
        $model = new SimplePurchaseOrder();
    ...
    {
        $model = self::with('account_info')->with('SimpleSupplier')->with('SimplePurchaseOrderGoods');
    ...
    public static function getPaginage($where,$search_data,$paginate,$orderby,$sort='DESC'){
        $model = self::with('SimpleSupplier')->with('account_info');
        ...
        return $model->with('SimplePurchaseOrderGoods')->where($where)->orderBy($orderby,$sort)->paginate($paginate);
    }
}
?>