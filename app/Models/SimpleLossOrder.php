...
class SimpleLossOrder extends Model{
    use SoftDeletes;
    protected $table = 'simple_loss_order';
    protected $primaryKey = 'id';
    public $timestamps = true;
    public $dateFormat = 'U';//设置保存的created_at updated_at为时间戳格式

    //和SimpleOrderGoods表一对多的关系
    public function SimpleLossOrderGoods(){
        return $this->hasMany('App\Models\SimpleLossOrderGoods','order_id','id');
    }
    ...
    {
        $model = self::with('SimpleLossOrderGoods');
    ...
    public static function getPaginage($where,$search_data,$paginate,$orderby,$sort='DESC'){
        ...
        return $model->with('SimpleLossOrderGoods')->where($where)->orderBy($orderby,$sort)->paginate($paginate);
    }
}
?>