...
class SimpleCheck
{
    public function handle($request, Closure $next)
    {
        ...
        switch ($route_name) {
            /*****登录页,如果已经登录则不需要再次登录*****/
            case "simple/login":                                //登录页,如果已经登录则不需要再次登录
            ...
            /****仅检测是否登录及是否具有权限****/
            case "simple":                             //后台首页
            case "simple/simple_list":                 //所有零售店铺列表
            case "simple/simple_switch":               //退出切换店铺
            case "simple/account/profile":             //账号信息
            case "simple/account/safe_password":       //安全密码设置
            case "simple/account/password":            //密码设置

            case "simple/category/category_add":       //添加商品分类
            case "simple/category/category_list":      //商品分类列表
            case "simple/goods/goods_add":             //添加商品
            case "simple/goods/goods_edit":            //编辑商品
            case "simple/goods/goods_list":            //商品列表
            ...
            case "simple/dispatch/dispatch_list":     //运费模板-运费模板列表
            case "simple/dispatch/dispatch_edit":     //运费模板-编辑运费模板

            case "simple/billing/purchase_goods":     //从供应商进货管理
            case "simple/billing/loss_goods":         //进销存管理--报损开单管理
            case "simple/billing/check_goods":        //进销存管理--盘点核对开单管理
            case "simple/billing/stock_list":         //进销存管理--库存列表

            case "simple/invoicing/purchase_goods":     //进销存管理--供应商到货开单
            case "simple/invoicing/return_goods":       //进销存管理--供应商退货开单
            case "simple/invoicing/loss_goods":         //进销存管理--报损开单
            case "simple/invoicing/check_goods":        //进销存管理--盘点核对开单

            case "simple/supplier/supplier_add":        //进销存管理--添加供应商
            case "simple/supplier/supplier_list":        //进销存管理--供应商列表

            case "simple/user/user_list":               //用户管理-粉丝用户管理

            case "simple/paysetting/payconfig":        //支付设置-收款信息设置
            case "simple/paysetting/shengpay_add":     //支付设置-添加终端机器号信息
            case "simple/paysetting/shengpay_list":    //支付设置-终端机器号信息列表
    ...
    public function checkIsLogin($request)
    {
        ...
        if (!empty($sess_key)) {
            ...
            $menu_data = Redis::get('zerone_system_menu_12_' . $sess_key);
            $son_menu_data = Redis::get('zerone_system_son_menu_12_' . $sess_key);
            ...
            return self::res(0, redirect('simple/login'));
        }
    }
