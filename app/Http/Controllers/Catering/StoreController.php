...
class StoreController extends Controller{
    ...
    public function branch_create(Request $request){
        ...
        $onebranch = Organization::checkRowExists([['parent_id',$organization_id],['type',5]]);
        dump($onebranch);
        $package_program = Package::getList(['id'=>1],0,'id','DESC');   //查询当前所选餐包含的程序 1为餐饮系统
        return view('Catering/Store/branch_create',['onebranch'=>$onebranch,'package_program'=>$package_program,'admin_data'=>$admin_data,'route_name'=>$route_name,'menu_data'=>$menu_data,'son_menu_data'=>$son_menu_data]);
        }
