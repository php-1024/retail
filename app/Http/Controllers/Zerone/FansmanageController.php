...
//        $list = Organization::getPaginageFansmanage1($where, $mobile, '10', 'id');
        $list = Organization::where($where)->join('account','organization.id','=','account.organization_id',function ($join) use ($mobile){
            $join->where('deepth','=','1');
            if($mobile){
                $join->where('account.mobile','=',$mobile);
            }
        })->select('organization.id,organization.status')->orderBy('organization.id', 'DESC')->paginate('10');
