...
class SubordinateController extends Controller{
    ...
    public function subordinate_add(Request $request){
        ...
        return view('Proxy/Subordinate/subordinate_add',['module_node_list'=>$module_node_list,'role_list'=>$role_list,'admin_data'=>$admin_data,'route_name'=>$route_name,'menu_data'=>$menu_data,'son_menu_data'=>$son_menu_data]);
    }
    ...
    public function quick_rule(Request $request){
        ...
        if($role_id <> '0'){
            $node_list = ProgramModuleNode::getRoleModuleNodes(2,$role_id);//获取当前角色拥有权限的模块和节点
            foreach($node_list as $key=>$val){
                $selected_modules[] = $val->module_id;
                $selected_nodes[] = $val->node_id;
            }
        }
        return view('Proxy/Subordinate/quick_rule',['module_node_list'=>$module_node_list,'selected_nodes'=>$selected_nodes,'selected_modules'=>$selected_modules]);
    }
