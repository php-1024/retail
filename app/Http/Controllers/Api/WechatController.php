   ...
   */
    ...
    */
    ...
    public function defined_menu(Request $request){
        ...
        $son_menu_data = $request->get('son_menu_data');//中间件产生的管理员数据参数
        $route_name = $request->path();//获取当前的页面路由
        $re = WechatAuthorization::getOne([['organization_id',$admin_data['organization_id']]]);
        dump($re);
        $list = WechatDefinedMenu::getList([['organization_id',$admin_data['organization_id']]],0,'id','DESC');
        dump($list);
        return view('Wechat/Catering/defined_menu',['list'=>$list,'admin_data'=>$admin_data,'route_name'=>$route_name,'menu_data'=>$menu_data,'son_menu_data'=>$son_menu_data]);
    }
