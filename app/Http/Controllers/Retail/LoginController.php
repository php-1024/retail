...
class LoginController extends Controller
{
    ...
    {
        dump(Session::get('retail_system_captcha'));
    ...
    public function captcha()
    {
        ...
        //把内容存入session
        Session::put('retail_system_captcha', $phrase);
        //生成图片
    ...
    public function login_check()
    {
        ...
        if ($account_info->id == 1) {
            ...
            $key = config("app.retail_encrypt_key");//获取加密盐（分店专用）
        }
            ...
            if (!empty($account_info)) {
                if ($encryptPwd != $account_info->password) {//查询密码是否对的上
                    ...
                    if ($account_info->id <> 1) {//如果不是admin这个超级管理员
                        if ($account_info->organization->asset_id <> '10') {//如果账号不属于餐饮分店平台管理系统，则报错，不能登录。5、是餐饮分店管理系统的ID
                            ErrorLog::addErrorTimes($ip, 10);
                            return response()->json(['data' => '登录账号、手机号或密码输入错误', 'status' => '0']);
                        } else {
