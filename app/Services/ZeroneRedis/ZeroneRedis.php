...
class ZeroneRedis
{
    ...
    /*
     * super_company超级管理员专用内部方法，生成商户系统账号的菜单
     * id - 用户的ID
     */
    public static function create_super_company_menu_cache($id){
        $menu = ProgramMenu::getList([[ 'parent_id',0],['program_id','3']],0,'id','asc');//获取商户系统的一级菜单
        $son_menu = [];
        foreach($menu as $key=>$val){//获取一级菜单下的子菜单
            $son_menu[$val->id] = ProgramMenu::son_menu($val->id);
        ...
        $son_menu = serialize($son_menu);
        Redis::connection('super_company');//连接到我的redis服务器——商户平台使用
        $menu_key = 'super_company_system_menu_'.$id;  //一级菜单的Redis主键。
        $son_menu_key = 'super_company_system_son_menu_'.$id;//子菜单的Redis主键
        Redis::set($menu_key,$menu);
        Redis::set($son_menu_key,$son_menu);
    }
}
